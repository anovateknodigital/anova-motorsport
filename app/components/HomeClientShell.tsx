...
import {
  MapPin, Calendar, Clock, ShieldCheck, Timer, FileText,
} from "lucide-react";
...
function RaceIntro({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const t3 = setTimeout(() => setPhase(4), 1650);
    const t4 = setTimeout(() => setPhase(5), 2200);
    const t5 = setTimeout(() => { onDoneRef.current(); }, 2850);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, transparent 40%, rgba(211,47,47,0.04) 50%, transparent 60%)" }}
      />
      <div
        className="relative mb-10 text-center"
        style={{ transition: "opacity 0.5s ease, transform 0.5s ease", opacity: phase >= 0 ? 1 : 0, transform: phase >= 0 ? "translateY(0)" : "translateY(16px)" }}
      >
        <p className="font-teko text-6xl md:text-7xl font-bold uppercase italic text-white tracking-tight leading-none" style={{ textShadow: "0 0 40px rgba(211,47,47,0.4)" }}>ANOVA</p>
        <div className="flex items-center gap-3 justify-center mt-1">
          <div className="h-[2px] w-8 bg-[#D32F2F]" />
          <p className="font-teko text-2xl md:text-3xl font-bold uppercase tracking-[0.35em] text-[#D32F2F]">MOTORSPORT</p>
          <div className="h-[2px] w-8 bg-[#D32F2F]" />
        </div>
      </div>
            ...
            <div key={light} className="flex flex-col items-center gap-3">
              ...
              <div
                style={{
                  ...
                  boxShadow: isOn ? "0 0 14px #D32F2F, 0 0 36px rgba(211,47,47,0.5)" : "none",
      ...
      <div style={{ opacity: phase >= 4 ? 1 : 0, transform: phase >= 4 ? "translateY(0)" : "translateY(12px)", transition: "opacity 0.2s ease, transform 0.2s ease" }}>
        <p className="font-teko text-5xl font-bold text-[#D32F2F] uppercase mt-8 tracking-[0.3em]" style={{ animation: phase >= 4 ? "goFlash 0.35s ease 4" : "none" }}>GO!</p>
      </div>
    </div>
  );
}

// ── Countdown for a single event ──────────────────────────────────
...
// ── Client-rendered Event Card ────────────────────────────────────
function EventCard({ evt, delay = 0 }: { evt: EventItem; delay?: number }) {
  function fmtRange(s: string, e: string) {
    const opts: Intl.DateTimeFormatOptions = { day: "numeric", month: "long", year: "numeric" };
    const start = new Date(s).toLocaleDateString("id-ID", opts);
    const end = new Date(e).toLocaleDateString("id-ID", opts);
    ...
    >
      <div className="relative w-full group">
        ...
        {evt.overlayImage && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 z-20 pointer-events-none opacity-60 group-hover:opacity-90 group-hover:scale-110 transition-all duration-500">
            <Image src={evt.overlayImage} alt="Overlay" fill className="object-contain brightness-0 invert" />
          </div>
        )}
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 text-zinc-300">
          <div className="flex items-center gap-3"><Calendar className="text-anova-red shrink-0" size={20} /><span>{fmtRange(evt.startDate, evt.endDate)}</span></div>
          {evt.timeInfo && <div className="flex items-center gap-3"><Clock className="text-anova-red shrink-0" size={20} /><span>{evt.timeInfo}</span></div>}
          {evt.location && <div className="flex items-start gap-3 sm:col-span-2"><MapPin className="text-anova-red shrink-0 mt-0.5" size={20} /><span>{evt.location}</span></div>}
        </div>

        {/* Live countdown */}
        <EventCountdown targetIso={evt.startDate} />
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <Link
            href="/events"
            className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-white py-3 rounded font-bold uppercase tracking-wider text-sm transition-all hover:scale-[1.02] active:scale-95 text-center block"
          >
            Detail &amp; Regulasi
          </Link>
        </div>
...
// ── Main Client Shell ─────────────────────────────────────────────
export default function HomeClientShell({ upcomingEvents }: { upcomingEvents: EventItem[] }) {
  ...
  useEffect(() => {
    async function loadSettings() {
      ...
      if (data) setSettings(data as Record<string, unknown>);
    }
    loadSettings();
  }, []);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center">
      ...
      {!pageReady && !showIntro && <div className="fixed inset-0 bg-zinc-950 z-[998]" />}
      ...
      {pageReady && (
        <div className="fixed top-0 left-0 right-0 h-[3px] bg-zinc-900/80 z-[100] pointer-events-none">
          <div
            ...
            style={{ width: `${scrollProgress}%`, transition: "width 60ms linear", boxShadow: scrollProgress > 2 ? "0 0 8px rgba(211,47,47,0.7)" : "none" }}
          />
        </div>
      )}

      {pageReady && (
        <div className="w-full flex flex-col items-center" style={{ animation: "fadeInPage 0.6s cubic-bezier(0.16, 1, 0.3, 1) both" }}>
          <Header />
          ...
          <section className="relative w-full h-[80vh] min-h-[600px] flex items-center justify-center overflow-hidden">
            <div className="absolute inset-0">
              <Image
                ...
                alt="Motoprix Action" fill className="object-cover scale-105"
                style={{ animation: "heroZoom 12s ease-in-out infinite alternate" }} priority
              />
              <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/50 to-transparent" />
              <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(105deg, transparent 30%, rgba(211,47,47,0.06) 50%, transparent 70%)" }} />
            </div>
            <div className="relative z-10 w-full max-w-7xl mx-auto px-4 md:px-8 text-white">
              <div className="max-w-2xl">
                <div data-reveal="left" className="inline-flex items-center gap-2 bg-[#D32F2F]/20 border border-[#D32F2F]/40 text-[#D32F2F] text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full mb-5">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#D32F2F] animate-pulse" />
                  Penyelenggara Resmi IMI
                </div>
                <h2 data-reveal="left" data-delay="1" className="font-teko text-5xl md:text-8xl font-bold uppercase italic leading-[0.9] text-white drop-shadow-lg mb-6">
                  Garis Finish Adalah<br />Awal Perjuangan
                </h2>
                <p data-reveal="left" data-delay="2" className="text-lg md:text-xl text-zinc-200 mb-10 max-w-xl leading-relaxed">
                ...
                <div data-reveal="left" data-delay="3" className="flex flex-col sm:flex-row gap-4">
                  <Link href="/events" className="bg-anova-red hover:bg-anova-red-hover text-white px-8 py-3.5 rounded font-bold uppercase tracking-wider text-sm transition-all text-center shadow-red-500/20 shadow-xl hover:scale-105 active:scale-95">
                    Daftar Event Sekarang
                  </Link>
                  <Link href="/results" className="bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white px-8 py-3.5 rounded font-bold uppercase tracking-wider text-sm transition-all text-center hover:scale-105 active:scale-95">
          ...
          <div className="w-full bg-black border-b border-zinc-800">
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 flex flex-wrap justify-between gap-8 text-center sm:text-left">
              {[
                ...
                <div key={label} data-reveal data-delay={String(i + 1)} className="flex flex-col flex-1 min-w-[150px] items-center">
          ...
          <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-20 grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-12 lg:gap-20">
            ...
            <div className="space-y-20">
              ...
              <section id="events" className="scroll-mt-32">
                ...
                {upcomingEvents.length === 0 ? (
                  <div className="py-10 border border-zinc-800 border-dashed rounded-xl text-center text-zinc-500">
                    ...
                    <Link href="/events" className="block mt-4 text-anova-red hover:underline text-sm">Lihat semua event →</Link>
                  </div>
                ) : (
                  <div className="space-y-10">
                    {upcomingEvents.map((evt, i) => (
                      <EventCard key={evt.id} evt={evt} delay={i} />
                    ))}
                  </div>
                )}
              </section>
              ...
              {/* Why Join Us */}
              <section>
                <h3 data-reveal="left" className="text-2xl font-bold text-white mb-8 flex items-center gap-3">Why Join Us?</h3>
                <div className="space-y-6">
                  {[
                    ...
                    <div key={title} data-reveal="left" data-delay={String(i + 1)} className="flex items-start gap-4 group">
            ...
            <div className="space-y-20">
              ...
              {/* Latest Race Results */}
              <section id="results">
                <h3 data-reveal="right" className="text-2xl font-bold text-white mb-8 flex items-center gap-3">Latest Race Results</h3>
                <div className="flex flex-col gap-5">
                  <div data-reveal="scale" className="relative bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 flex items-center justify-between p-6 pr-0 group hover:border-anova-red/50 transition-colors shadow-2xl">
                    ...
                    <div className="relative z-10 h-32 w-1/3 min-w-[120px] rounded-l-[40px] overflow-hidden -mr-2 bg-black border-l-4 border-anova-red drop-shadow-2xl translate-x-2 group-hover:translate-x-0 transition-transform duration-500">
                      <Image src="https://asset.kompas.com/crops/_WPT6_jc87U-nz1_jNlkjTcx2Dg=/0x0:1599x1066/750x500/data/photo/2024/11/17/673a0716d9b2b.jpeg" alt="Bintang N." fill className="object-cover object-top scale-110 group-hover:scale-125 transition-transform duration-700" />
                  ...
                  <div data-reveal="scale" data-delay="1" className="relative bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 flex items-center justify-between p-6 pr-0 group hover:border-anova-red/50 transition-colors shadow-2xl">
                    ...
                    <div className="relative z-10 h-32 w-1/3 min-w-[120px] rounded-l-[40px] overflow-hidden -mr-2 bg-black border-l-4 border-anova-red drop-shadow-2xl translate-x-2 group-hover:translate-x-0 transition-transform duration-500">
                      <Image src="https://cdn.grid.id/crop/0x0:0x0/700x465/photo/2020/09/28/4272447586.jpg" alt="Reza V." fill className="object-cover object-center scale-110 group-hover:scale-125 transition-transform duration-700" />
                      <div className="absolute inset-0 bg-gradient-to-r from-anova-red/30 to-transparent mix-blend-overlay" />
                    </div>
                  </div>
              ...
              {/* Gallery */}
              <section id="gallery">
                <h3 data-reveal="right" className="text-2xl font-bold text-white mb-8 flex items-center gap-3">Media Gallery</h3>
          ...
          {/* Partners — dynamic from Supabase */}
          {sponsors.length > 0 && (
                        ...
                        s.website_url ? (
                          ...
                          >
                            <Image
                              ...
                              unoptimized={s.logo_url.startsWith("http")}
                            />
                          </a>
                        ) : (
                          <Image
